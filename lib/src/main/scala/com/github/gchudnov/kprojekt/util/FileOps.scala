...
object FileOps {
  ...
  def createTempDir(prefix: String): Either[Throwable, File] = Either.catchNonFatal {
    Files.createTempDirectory(prefix).toFile
  }

  def stringFromFile(file: File): Either[Throwable, String] = Either.catchNonFatal {
    Using.resource(Source.fromFile(file)) { file =>
      file.getLines.mkString("\n").trim()
    }
  }

  def stringFromResource(resourcePath: String): Either[Throwable, String] = Either.catchNonFatal {
    Using.resource(Source.fromResource(resourcePath)) { source =>
      source.getLines.mkString("\n").trim()
    }
  }

  def saveString(file: File)(data: String): Either[Throwable, Unit] = Either.catchNonFatal {
